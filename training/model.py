...
class StemBlock(nn.Module):
    ...
    def __init__(self):
        ...
        self.fuse = ConvBNReLU(32, 8, 3, stride=1)
...
class CEBlock(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_gap = ConvBNReLU(32, 32, 1, stride=1, padding=0)
        #TODO: in paper here is naive conv2d, no bn-relu
        self.conv_last = ConvBNReLU(32, 32, 3, stride=1)
...
class BGALayer(nn.Module):
    ...
    def __init__(self):
        ...
        self.up1 = nn.Upsample(size=(10, 20))
        self.up2 = nn.Upsample(size=(10, 20))
...
class SegmentHead(nn.Module):
    ...
    def __init__(self, in_chan, mid_chan, n_classes, up_factor=8, aux=True):
        ...
        self.conv_out = nn.Sequential(
            ...
            nn.Upsample(size = (80, 160), mode='bilinear', align_corners=False)
        )
...
class BiSeNetV2(nn.Module):
    ...
    def __init__(self, n_classes, aux_mode='train'):
        ...
        self.head = SegmentHead(32, 32, n_classes, up_factor=8, aux=False)
        if self.aux_mode == 'train':
            self.aux2 = SegmentHead(8, 64, n_classes, up_factor=4)
            self.aux3 = SegmentHead(8, 64, n_classes, up_factor=8)
            self.aux4 = SegmentHead(16, 64, n_classes, up_factor=16)
            self.aux5_4 = SegmentHead(32, 64, n_classes, up_factor=32)

        # self.init_weights()

    def forward(self, x):
        size = x.size()[2:]
        feat_d = self.detail(x)#32
        feat2, feat3, feat4, feat5_4, feat_s = self.segment(x) #32
...
if __name__ == "__main__":
    ...
    # x = torch.randn(16, 3, 1024, 2048)
    model = BiSeNetV2(n_classes=3).cuda()
