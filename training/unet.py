...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, up = False, mid_channels=None):
        super().__init__()
        if not mid_channels:
            mid_channels = out_channels
        if in_channels != 3 and not up:
            self.double_conv = nn.Sequential(
                nn.Conv2d(in_channels, mid_channels, kernel_size=3, padding=1, bias=False, groups=in_channels),
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv = DoubleConv(in_channels, out_channels, in_channels // 2, up = True)
        else:
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)
            self.conv = DoubleConv(in_channels, out_channels, up = True)
